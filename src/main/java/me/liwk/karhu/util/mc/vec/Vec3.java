...
public class Vec3 {
    ...
    public Vec3 normalizeModern() {
        double d0 = Math.sqrt((xCoord * xCoord) + (yCoord * yCoord) + (zCoord * zCoord));
    ...
    public Vec3 scale(double mult) {
        return this.multiply(mult, mult, mult);
    }

    public Vec3 multiply(double multX, double multY, double multZ) {
        return new Vec3(this.xCoord * multX, this.yCoord * multY, this.zCoord * multZ);
    }
