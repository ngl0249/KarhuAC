...
public final class KarhuThreadManager {
    ...
    public static ExecutorService createNewExecutor(int no) {
        final ExecutorService service = Executors.newFixedThreadPool(no);
        activeServices.add(service);
        return service;
    }

    public static ExecutorService createNewExecutor(int no, String name) {
        final ExecutorService service = Executors.newFixedThreadPool(no, new ThreadFactoryBuilder()
                .setNameFormat(name)
                .build());
        activeServices.add(service);
        return service;
    ...
    public static ScheduledExecutorService createNewScheduledExecutor(int no, String name) {
        final ScheduledExecutorService service = Executors.newScheduledThreadPool(no, new ThreadFactoryBuilder()
                .setNameFormat(name)
                .build());
        activeServices.add(service);
        return service;
