...
import static me.liwk.karhu.menu.ChecksMenuLegacy.openCheckSettingGUI;
...
public class MChecksMenu {
    ...
    public static void openMainMenu(Player opener) {
        ...
        Gui gui = new Gui(ChatColor.translateAlternateColorCodes('&',
                highlight + "&l" + acname + "&7 - Checks"),
                5 * 9);
        ...
        for (SubCategory categoryShit : SubCategory.values()) {
            ...
            gui.addButton(new Button(1, categoryShit.getSlot(), ItemUtil.makeItem(type, 1, cfg.getGuiHighlightColor() + name, Arrays.asList(
                ...
                public void onClick(Player clicker, ClickType clickType) {
                    gui.close(clicker);
                    openCheckSettingGUI(clicker, categoryShit);
                }
            });
        }

        gui.addButton(new Button(1, 40, ItemUtil.makeItem(Material.EMERALD, 1, cfg.getGuiHighlightColor() + "Back", Arrays.asList(
                "§7§m⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤⏤",
