    ...
    public void onCommand(CommandArgs command) {
        ...
        if (command.getLabel().equalsIgnoreCase("alerts")) {
            ...
            if (command.getSender() instanceof Player) {
                ...
                player.sendMessage(Karhu.getInstance().getAlertsManager().hasAlertsToggled(player.getUniqueId()) ? (ChatColor.translateAlternateColorCodes('&', cfg.getConfig().getString("commands.alerts.enabled"))) : (ChatColor.translateAlternateColorCodes('&', cfg.getConfig().getString("commands.alerts.disabled"))));
            }
        }
    }
