...
public class PlayerVelocityHandler implements Listener {
    ...
    public void onPlayerVelocity(PlayerVelocityEvent event) {
        if (!event.isCancelled()) {
            ...
            if (data != null && data.isReadyToAccept()) {
    ...
    public void onExplode(EntityExplodeEvent event) {
        if (!event.isCancelled()) {
            ...
            for (Entity entity : event.getEntity().getNearbyEntities(4D, 4D, 4D)) {
                if (entity instanceof Player && !entity.hasMetadata("NPC")) {
                    KarhuPlayer data = Karhu.getInstance().getDataManager().getPlayerData(entity.getUniqueId());
