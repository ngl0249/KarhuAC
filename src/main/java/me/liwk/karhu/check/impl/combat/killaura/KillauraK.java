...
@CheckInfo(name = "Killaura (K)", category = Category.COMBAT, subCategory = SubCategory.KILLAURA, experimental = false, credits = "§c§lCREDITS: §aMexify §7made this check.")
    ...
    public void handle(final Event packet) {
        if (packet instanceof DigEvent) {
            ...
            if(data.getClientVersion().getProtocolVersion() <= 47) {
                if(type == DiggingAction.RELEASE_USE_ITEM && (data.isPlacing() || data.isUsingItem())) {
                    fail("* Illegal block order", getBanVL(), 120);
                }
            }
        }
    }
