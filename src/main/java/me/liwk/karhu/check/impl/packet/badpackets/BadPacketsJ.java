...
@CheckInfo(name = "BadPackets (J)", category = Category.PACKET, subCategory = SubCategory.BADPACKETS, experimental = false)
public final class BadPacketsJ extends PacketCheck {
    ...
    public void handle(final Event packet) {
        if(packet instanceof HeldItemSlotEvent) {
            if (((HeldItemSlotEvent) packet).getSlot() < 0) {
                fail("* Negative slot packet", getBanVL(), 1);
            }
        }
    }
}