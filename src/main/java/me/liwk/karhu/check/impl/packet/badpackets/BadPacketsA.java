...
@CheckInfo(name = "BadPackets (A)", category = Category.PACKET, subCategory = SubCategory.BADPACKETS, experimental = false)
    ...
    public void handle(final Event packet) {
        if(packet instanceof FlyingEvent) {
            if (((FlyingEvent) packet).hasMoved() || ((FlyingEvent) packet).hasLooked()) {
                if (Math.abs(((FlyingEvent) packet).getPitch()) > 90 && !data.isPossiblyTeleporting()) {
